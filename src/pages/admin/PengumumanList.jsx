      ...
      <div className="sticky top-0 z-50" style={{ background: '#1E3A5F' }}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white font-bold text-sm">Admin - Pengumuman</p>
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: '#FFFFFF', border: '1px solid #E2E8F0' }}>
                ...
                <tbody>
                  {filtered.map((p) => (
                    <tr key={p.id} className="border-t" style={{ borderColor: '#F1F5F9' }}>
                      <td className="px-4 py-3 font-medium" style={{ color: '#0F172A' }}>{p.judul}</td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <Link to={`/admin/pengumuman/edit/${p.id}`} className="p-2 rounded-lg" style={{ color: '#64748B' }}>
                            <Edit3 className="w-4 h-4" />
                          </Link>
                          <button onClick={() => handleDelete(p.id)} className="p-2 rounded-lg" style={{ color: '#DC2626' }}>
            ...
            {filtered.length === 0 && (
              <div className="text-center py-16">
                ...
                <Link to="/admin/pengumuman/tambah" className="text-sm font-semibold hover:underline mt-1 inline-block" style={{ color: '#1E3A5F' }}>Tambah pengumuman sekarang</Link>
              </div>
            )}
          </div>
        )}
