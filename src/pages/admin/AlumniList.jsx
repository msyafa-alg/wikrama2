...
const AlumniList = () => {
  ...
  const fetchAlumni = async () => {
    setLoading(true)
    const { data } = await supabase.from('alumni').select('*').order('nama')
    setAlumni(data || [])
    setLoading(false)
  }
  ...
  const handleDelete = async (id) => {
    if (!confirm('Yakin hapus alumni ini?')) return
    const a = alumni.find(s => s.id === id)
    if (a?.foto) {
      const path = a.foto.replace(/.*\/student-files\//, '')
      if (path) await supabase.storage.from('student-files').remove([path])
    }
    await supabase.from('alumni_details').delete().eq('id', id)
    await supabase.from('alumni').delete().eq('id', id)
    setAlumni(prev => prev.filter(s => s.id !== id))
    toast('Alumni berhasil dihapus', 'success')
  }

  const angkatanList = ['semua', ...new Set(alumni.map(a => a.angkatan).filter(Boolean))]

  const filtered = alumni.filter(a => {
    const matchAngkatan = filterAngkatan === 'semua' || a.angkatan === filterAngkatan
      ...
      <div className="sticky top-0 z-50" style={{ background: '#1E3A5F' }}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white font-bold text-sm">Admin - Alumni</p>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Cari alumni..." value={search}
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: '#FFFFFF', border: '1px solid #E2E8F0' }}>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ background: '#F8FAFC' }}>
                    ...
                    <th className="text-left px-4 py-3 font-semibold" style={{ color: '#64748B' }}>Angkatan</th>
                    <th className="text-left px-4 py-3 font-semibold" style={{ color: '#64748B' }}>Aktivitas</th>
                ...
                <tbody>
                  {filtered.map((a) => (
                    <tr key={a.id} className="border-t" style={{ borderColor: '#F1F5F9' }}>
                      <td className="px-4 py-3"><img src={a.foto || '/favicon.png'} alt="" className="w-10 h-10 rounded-xl object-cover" /></td>
                      <td className="px-4 py-3 font-medium" style={{ color: '#0F172A' }}>{a.nama}</td>
                      <td className="px-4 py-3" style={{ color: '#64748B' }}>{a.angkatan}</td>
                      <td className="px-4 py-3" style={{ color: '#64748B' }}>{a.aktivitas}</td>
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <Link to={`/admin/alumni/edit/${a.id}`} className="p-2 rounded-lg" style={{ color: '#64748B' }}>
                            <Edit3 className="w-4 h-4" />
                          </Link>
                          <button onClick={() => handleDelete(a.id)} className="p-2 rounded-lg" style={{ color: '#DC2626' }}>
            ...
            {filtered.length === 0 && (
              <div className="text-center py-16">
                <p className="font-medium" style={{ color: '#94A3B8' }}>Tidak ada alumni</p>
                <Link to="/admin/alumni/tambah" className="text-sm font-semibold hover:underline mt-1 inline-block" style={{ color: '#1E3A5F' }}>Tambah alumni sekarang</Link>
              </div>
            )}
          </div>
        )}
